...
## 研究问题：用户反馈的消费语义（RQ-6）
- 状态: **planned**（决策已定：实例级 exemplar 消费 + 范例→模板上升路径，见 DECISIONS.md [2026-08-28]）
- 日期: 2026-08-28
...
- 备注: 2026-08-28 对话产出；原以「用户自创菜谱的存储与复用」记入（d3963a3），后经用户澄清本质上抽象为「反馈如何被消费」，升维为研究问题 RQ-6。下一步：与用户对齐「实例级消费」语义（当 in-context 范例 vs 当结构模板约束），再落 DECISIONS.md。
