...
## 用户自创菜谱（拿手菜 / favorite dishes）的存储与复用
- 状态: **idea**
- 日期: 2026-08-28
- 动机: 用户自己尝试做了一道菜，评价「又简单又营养、味道特别新鲜」，想保存下来复用。当前 skill 没有「用户验证过的具体菜」的落点——`pairingTemplates` 存的是「结构模板」（哪一餐=哪些类别），不存「具体菜」；`feedback.json` 是原始日志而非复用资产；通用知识层（ingredient_knowledge.md）刻意守「不写死固定菜谱」边界（防僵化、防与库存/画像冲突）。缺一个「用户个人拿手菜」的落点。
- 评估:
  - 方案 A（复用 pairingTemplates，零 schema 改动）：把具体菜抽象成「一组食材 + 一个做法」写进 `pattern`，每日搭配的模板层天然消费它（复用闭环）。语义从「结构」放宽到「结构+具体菜」，菜少时够用。
  - 方案 B（profile 新增轻量字段如 `favoriteDishes` / `myRecipes`）：语义更精确（区分「结构模板」vs「具体拿手菜」），但要在 schema.md 补字段定义、SKILL.md 搭配流程与反馈落点做相应说明。适合菜会积累多道、且不想混淆结构模板的情况。
  - 方案 C（先记 feedback 流水，暂不沉淀成资产）：最轻，但 feedback 是 log 不是资产；验证过的好菜最终应升级到 profile 持久字段，否则停留流水无法被搭配层直接复用。
  - 成本 A < C < B；收益 A（零成本进复用闭环）与 B（语义最干净）都高于 C。风险：语义混淆（结构 vs 具体菜）；须澄清「不写死菜谱」边界只约束通用层——用户个人菜谱属用户数据，与边界不冲突。
- 备注: 2026-08-28 对话产出；与「食材咨询入口」（2026-08-15）同属「个性化×通用」分离原则的延续，但这次落点是「用户个人具体菜」而非「通用食材知识」。倾向方案 A 先复用 pairingTemplates（零改动、立即可用），待拿手菜积累到多道再评估是否拆独立字段。待用户选定方案并补充分菜品内容后再进入 planned/implemented。
