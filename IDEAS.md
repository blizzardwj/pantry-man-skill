...
## 研究问题：用户反馈的消费语义（RQ-6）
- 状态: **planned**（决策已定：实例级 exemplar 消费 + 范例→模板上升路径，见 DECISIONS.md [2026-09-08]）
- 日期: 2026-09-08
...
- 备注: 2026-09-08 对话产出；原以「用户自创菜谱的存储与复用」记入（d3963a3），后经用户澄清本质上抽象为「反馈如何被消费」，升维为研究问题 RQ-6。下一步：与用户对齐「实例级消费」语义（当 in-context 范例 vs 当结构模板约束），再落 DECISIONS.md。
...
- 评估: 与 RQ-5 已有「耗尽候选」（负向吃完→补货）拼成采购计划"反馈驱动补货"的两翼。规则草案：① 生成采购计划读 `profile.exemplars`，短周期生鲜主料不在食材池 → 优先推荐进对应品类并标注「你的拿手菜 X 需要它」（明示因果，不说"我会保供"）；长周期主料沿用 stock-aware skip。② **软倾向不硬约束**：仍受 quantity benchmark + 品类多样性约束，用户在确认闸门可删。
- 备注: 2026-09-08 对话产出；属 RQ-6「用户反馈的消费语义」的**第二消费出口**（第一出口 = 每日搭配复用）。待用户拍板后实施 SKILL.md 采购计划步骤。
